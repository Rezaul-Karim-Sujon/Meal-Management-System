...
export default function Meal() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:12269/api/Meals",{"companyId":companyId})
      .then((res) => {
        dispatch(updateMealListAction(res))
      })
      .catch((err) => {
        console.log(err);
      });
    ...
    <div className="menulist ">
      <div className="itemHeaderDiv row">
        <div className="itemAddBtnDiv ">
          <Link to="/adminDashboard/createMeal"><button
          ...
          <div class="accordion-item">
            <h2 class="accordion-header" id="headingOne">
              <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                ...
                {meal.mealType==1?"Breakfast":(meal.mealType==2?"Lunch":"Dinner")}
              </button>
            </h2>
            <div id="collapseOne" class="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
              ...
              {menuList.map((key2,menuObj)=>{
                return(
                  (menuObj.menuId === meal.menuId && menuObj.fixedItem === true)?
              ...
              {menuList.map((key2,menuObj)=>{
                return(
                  (menuObj.menuId === meal.menuId && menuObj.fixedItem === false) ?
                  <div className="d-flex">
                    <div>{menuObj.menuItemFoodItems.picture}</div>
                    <div>{menuObj.menuItemFoodItems.recipeName}</div>
